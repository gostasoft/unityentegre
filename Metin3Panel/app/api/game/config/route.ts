...
export async function GET(request: NextRequest) {
  ...
  const [settingsRows, maps, entities, items, spawns, worldPlacements, drops, shops, shopItems, events, sanctions, revision] = await Promise.all([
    ...
    database().prepare(`SELECT
      ...
      (SELECT COUNT(*) FROM world_placements) AS version`).first<{version:string}>(),
  ]);
  ...
  return NextResponse.json({ version: revision?.version ?? '0', settings, maps, entities, runtimeEntities, items, spawns, worldPlacements, groups: requestedGroups, drops, shops, shopItems, events, sanctions });
}
