...
export function Metin3AdminPanel({ user, initialView='dashboard' }: { user: { name: string; email: string }; initialView?: string }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen?'mobile-open':''}`}>
      ...
      <nav className="nav-list"><p className="nav-section">PROTO KATALOĞU</p><a className="nav-item" href="/panel/mobs"><Skull size={18}/><span>Moblar</span></a><a className="nav-item" href="/panel/metins"><Gem size={18}/><span>Metinler</span></a><a className="nav-item" href="/panel/items"><PackageOpen size={18}/><span>İtemler</span></a><p className="nav-section">YÖNETİM</p>{nav.map(([id,label,Icon])=><a href={`/panel?view=${id}`} key={id} className={`nav-item ${view===id?'active':''}`} onClick={(event)=>{event.preventDefault();setView(id);history.replaceState(null,'',`/panel?view=${id}`);setMobileOpen(false)}}><Icon size={18}/><span>{label}</span>{id==='players'&&<b>{onlinePlayers}</b>}</a>)}<PanelExtendedNav/></nav>
      <div className="server-card"><div className="server-title"><Activity size={16}/> Oyun API Bağlantısı</div><div className="server-row"><span><i/> Panel Veritabanı</span><strong>Hazır</strong></div><div className="server-row"><span><i/> Unity Eşitleme</span><strong>3 sn</strong></div><div className="server-health"><span style={{width:'96%'}}/></div><small>Son veri yenileme: şimdi</small></div>
...
function PlayersView({data,query,openNew,setModal,mutate}:any){const rows=data.players.filter((x:Row)=>(x.account+' '+x.character_name).toLowerCase().includes(query.toLowerCase()));return <><Heading eyebrow="HESAP GÜVENLİĞİ" title="Oyuncu Yönetimi" description="Bir oyuncuya tıklayarak envanterini, ekonomisini, harita geçmişini ve tüm yaptırımlarını açın." action={()=>openNew('players')} label="Oyuncu Kaydı"/><DataTable title={`${rows.filter((x:Row)=>x.online).length} çevrimiçi · ${rows.length} kayıt`} rows={rows} columns={[['character_name','Karakter'],['account','Hesap'],['empire','İmparatorluk'],['character_class','Sınıf'],['level','Lv.'],['online','Durum'],['last_map_code','Son Harita'],['yang','Yang']]} rowHref={(row:Row)=>`/panel/players/${row.id}`} edit={row=>setModal({resource:'players',data:row})} remove={row=>mutate('players','delete',{id:row.id})}/></>}
...
function DataTable({title,rows,columns,edit,remove,extra,rowHref}:any){return <article className="panel data-table"><div className="panel-head"><div><span className="panel-kicker">KAYITLAR</span><h2>{title}</h2></div></div><div className="table-scroll"><table><thead><tr>{columns.map((c:any)=><th key={c[0]}>{c[1]}</th>)}<th>İşlem</th></tr></thead><tbody>{rows.map((row:Row)=><tr key={row.id} className={rowHref?'clickable-row':''} tabIndex={rowHref?0:undefined} onClick={()=>rowHref&&(window.location.href=rowHref(row))} onKeyDown={event=>{if(rowHref&&(event.key==='Enter'||event.key===' '))window.location.href=rowHref(row)}}>{columns.map(([key]:any)=><td key={key}>{typeof row[key]==='number'&&key!=='id'?fmt(row[key]):key.includes('until')?dateText(row[key]):row[key]===1?<span className="status good">Aktif</span>:row[key]===0?<span className="status">Pasif</span>:String(row[key]??'—')}</td>)}<td className="row-actions" onClick={event=>event.stopPropagation()}><button onClick={()=>edit(row)} title="Düzenle"><Edit3/></button>{extra?.(row)}<button onClick={()=>{if(confirm('Bu kayıt silinsin mi?'))remove(row)}} title="Sil"><Trash2/></button></td></tr>)}</tbody></table>{!rows.length&&<div className="empty">Bu bölümde henüz kayıt yok.</div>}</div></article>}
